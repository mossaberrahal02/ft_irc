...
bool	isExist(std::vector<Client> &clients, std::vector<Client> &channel_clients)
{
	for (size_t i = 0; i < channel_clients.size(); i++)
		if (clients[i].fd_client == channel_clients[i].fd_client)
			return true;
	return false;
}
...
{
	if (isExist(clients, channels[index_channel].clients))
	{
		send_log(index_client, "JOIN : You Already in this channel\r\n");
		server_log(index_client, "JOIN : You Already in this channel");
		return;
	}
	...
	sendToAll(clients[index_client].fd_client, clients, channels[index_channel].admins,
...
bool isValidChannelName(const std::string &new_channel)
{
    ...
    if (new_channel.size() == 1)
        return false;

    // check remaining characters
    for (size_t i = 1; i < new_channel.size(); i++)
    {
