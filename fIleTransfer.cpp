...
void Server::sendFile(int index_client, std::vector<std::string> cmd_args)
{
    ...
    if (pos == std::string::npos)
        filename = cmd_args[2]; // No path separators, entire string is filename
    else
        filename = cmd_args[2].substr(pos + 1);

    // Create file object: (filename, filepath, sender, receiver)
...
{
    if (cmd_args.size() < 3) // Need GETFILE <filename> <destination_path>
    {
        send_to_client(clients[index_client].fd_client, ":Not enough parameters\r\n");
        return;
    }
    ...
    {
        send_to_client(clients[index_client].fd_client, ":No such file on the server\r\n");
        return;
    }
    ...
    // Check if current client is the intended receiver
    std::cout << "Current client: " << clients[index_client].nickName << " file receiver: " << file.receiver << std::endl;
    if (file.receiver != clients[index_client].nickName)
    {
        send_to_client(clients[index_client].fd_client, ":Permission Denied\r\n");
        return;
    }
    ...
    if (!ofs.is_open())
    {
        send_to_client(clients[index_client].fd_client, ":Cannot create output file\r\n");
        return;
    }
